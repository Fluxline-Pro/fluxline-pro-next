...
All tag/category/technology routes use URL slugs generated by `slugify()` in `src/utils/slug.ts`.

**Taxonomy routing convention:**

- `generateStaticParams()` returns slugs, not display names
- Pages resolve `slug -> display name` with `resolveSlug()`
- Navigation builds taxonomy URLs with `slugify()`
- Never use `encodeURIComponent()` for taxonomy paths
- `dynamicParams = false` remains required on taxonomy pages
- Slug collisions fail the build by design so conflicting taxonomy terms are fixed in content frontmatter
- Legacy space-named URLs are preserved with build-generated `301` redirects in `out/staticwebapp.config.json`

**To verify taxonomy routing locally:**

```bash
# Run in development
yarn dev

# Or verify the static export
yarn build
